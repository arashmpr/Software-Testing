...
public class Baloot {
    ...
    public ArrayList<Commodity> filterCommoditiesByProviderName(String id) {
        ArrayList<Commodity> result = new ArrayList<>();
        if (id == "")
            return result;
        for (Commodity commodity : Database.getInstance().getCommodities())
            if (commodity.getProviderId().contains(id))
                result.add(commodity);
...
//    public void rateCommodity(String commodityId, String username, int rate) throws NotExistentCommodity {
//        Commodity commodity = getCommodityById(commodityId);
